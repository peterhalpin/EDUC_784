...
<!-- In this chapter, make sure to review the math in section 1 and then do the worked example in class. --> 
...
If we apply the log-transformation to variable, we can always "undo" the transformation to get back to the original variable. This is important because we often want to interpret the results of an analysis in terms of the original variable, not the transformed one. We will see this trick a lot in what follows -- apply a transform to the data, run the model, then undo the transform to interpret the results. 

The way to undo the log transform is using exponentiation, which is abbreviated "$\exp$".  

The mathematical terminology here is that $\exp$ is the *inverse* of $\log$, and vice versa. The relationship is a bit slippery to grasp, but the overall idea is that we once we log transform a variable, we can use the inverse transformation ($\exp$) to get back to the original variable. So, $\log$ and $\exp$ are like best buddies -- whenever we see one of them, the other is bound to show up also. 
...
So, taking the log of $100$ gives us $4.60517$, and then taking the exponent of $4.60517$ gets us back to the original input, $100$. In general, $e^{\log(x)} = x$. 

This is what it means for $\exp$ to undo or invert the $\log$ function. Exponentiation (i.e., raising something to a power) undoes the logarithm. 

As mentioned, the important thing about logs and exponents is that we can always use one to undo the effects of the other. So, if we log transform our $Y$ variable in a regression analysis to make the data more compatible with the model assumptions, we can always undo the log transform afterwards by taking exponents. 
...
For example, say I loan $Y = 100$ dollars to a friend and they pay me back $Y' = 150$ dollars. The amount my friend paid me back is 1.5 times larger, or 150% larger, than what I initially lent them:
...
In more financial terms, I charged my friend 50% interest on the loan, and so they had to paid me back 150% of the principal (some friend!). My gains are 50%, so the final amount of I have is 150% larger than what I initially had. 

As another example, if $Y' = 40$ and $Y = 100$, then $Y'$ is 40% *as large as* $Y$ (relative magnitude) and the relative decrease from 100 to 40 is 60%. In the context of our example, my friend  paid me back 40% of what I lent them, so I lost 60% of my initial amount. 
...
Now that we know about proportion/percentage/relative change, let's use these concepts to interpret the regression coefficient in Equation \@ref(eq:logy). Recall that this interpretation is approximate and only applies when $|b| < 1$. 
...
Start with the log-linear model and then exponentiate both sides and then use the multiplication rule for exponents:
