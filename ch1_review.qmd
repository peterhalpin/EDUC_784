...
Please review up to the Exercises in @sec-exercizes-1 before the first
...
that the sum is over the all available values of $X$ (i.e., from $1$ to
$N$):
...
## Some properties of sample statistics {#sec-properties-1}
...
$d_i = X_i - \bar X$. It is always the case that $\sum_i d_i = 0$
...
statistic. These properties are defined in terms of the *sampling
...
sample data. It can be any statistic, but let's say the mean, $\bar X$,
for concreteness. 3. Write down the value of the mean, and then return
...
-   **The expected value** of the mean, denoted $E(\bar X)$, is the mean
    ...
    is why we say the expected value of a statistic rather than the mean
    of a statistic. It's called the expected value because it's the
...
estimate statistic is close the hypothesized value of the parameter,
...
approaches a normal distribution. This means that that the differences
...
relationship assumes we use the same level of $\alpha$ for both the test
...
**Before doing this section, scroll to the top of the page, click on the
"\</\> Code" menu, and make sure it is set to "Show All Code."**
...
-   R uses the hashtag symbol (`#`) for comments. Comments are ignored
...
-   R has some names that are reserved for built-in stuff, like `log`
    ...
    built-in until you delete that name (e.g., `rm(x)`).
...
To get started, lets load the `NELS` data. The data are a subsample of
...
the datasetby their names directly
