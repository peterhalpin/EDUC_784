...
In the previous chapter we considered how to check whether the population assumptions of linear regression are problematic for a given data set. In this chapter we discuss one way to deal with assumption violations --  by "transforming" the $Y$ variable. In general, a transformation is just a function we apply to variable to change it into another variable that we would rather work with. We have already seen one example of a transformation: the z-transformation or z-scores. 
...
Before we get into transforming our variables, let's review the requisite math -- logs and exponents. We will use the lowercase symbols $x$ and $y$ for generic mathematical variables in this section -- these symbols do not correspond to the use of the uppercase symbols $X$ and $Y$ in regression notation. So, e.g., $x$ is not a predictor, its just a generic variable. 
...
Regardless of which base we use, logs have the same overall compression effect, as shown in Figure \@ref(fig:loge). For example, $\log_{10}(1000) = 3$ and $\log_{e}(1000) \approx 7$ -- both numbers are much smaller than $1000$! The same is true when applied to real data -- in fact, the left hand panel in Figure \@ref(fig:logdata) was computed using $\log_e$. 
...
So, taking the log of $100$ gives us $4.60517$, and taking the exponent of $4.60517$ gets us back to the original input, $100$. In general, $e^{\log(x)} = x$. So, exponentiation (i.e., raising something to a power) undoes the logarithm. This undoing is called inversion.  
...
There are some rules for working with logs and exponents that we will use to derive some results on log-linear regression (and logistic regression, when we get there). You don't need to know these but you might find them useful if you like to follow derivations. 
...
Since the log-linear models are often used to address positive skew in the outcome variable, we will tackle these 3 points in order.
...
The upshot of all this is that, if your variable takes on values in the range $[0, 1]$, the log transform is going to change those values into large negative numbers. This can result in negative skew, rather fixing your positive skew. 
...
Censoring can look like extreme skew, but is importantly different from skew because a large proportion of the observations have exactly the same value. An example is shown in Figure \@ref(fig:floor) below. Note that the log transformation does do not anything about the censored data. In general, if you transform a "heap" of values, then the same heap shows up in the transformed data. 
...
What this means in practice is that addressing positive skew using a log transform can actually create more problems than it solves. Figure \@ref(fig:nonlinear1) illustrates this situation. In the top row, we see  the residual versus fitted plot for the regression of Math Achievement (`c1rmscal`) on SES (`wksesl`) in the ECLS data, which resulted in a clear violation of normality (positive skew). In the bottom row, we see same output after log transforming Math Achievement -- it fixed the issue with normality, but also made the nonlinearity more apparent.  
...
Note that the opposite can also happen -- a log transform can make a non-linear (exponential) relationship more linear. The exact result is of the transformation depends on the relationships in the data. 


As we discuss in the Sections \@ref(polynomial-9) and \@ref(piecewise-9), we can address non-linearity of regression by transforming the $X$ variable(s). But we should be sure that the problem we are trying to solve (normality) isn’t worse than the the problem we end up with (non-linearity). 
...
The final point about the log-linear model is how it affects the interpretation of the model parameters. For example, we know what a $b$-unit increase in $Y$ means, but how to we interpret a $b$-unit increase in $\log(Y)$? This is definitely the most important point about the log transformation, which is why we saved it for last. 

In general, one of the main shortcoming of "transforming your data" is that it can lead to uninformative or unclear interpretations of model parameters. If the results don't make sense, its not much of a consolation to know that the assumptions looked OK! 
...
In a log-linear model, the regression coefficients can be interpreted as approximate proportion change in $Y$, in the original units. So, in this equation, 
...
and multiplying this by 100 gives us the percentage change. For example, if $Y = 100$ and $Y' = 150$ then, 
...
Now that we know what proportion and percentage change are, the next thing to do is use these concepts to interpret the regression coefficient in Equation \@ref(eq:logy). Recall that this interpretation is approximate and only applies when $|b| < 1$. 
...
To get more of the flavor for interpreting the log-linear model, let's use a hypothetical example in which we compare it the the regular linear model, Let's use the variables 
...
Now let's instead consider a log-linear model, this time with a regression coefficient of $b = .2$:
...
This example shows that, while the linear model represents change in fixed units (e.g., dollars), the log-linear model represents change that is proportional to the magnitude of $Y$. Particularly in applications involving financial outcomes, it has been argued that people’s subjective evaluation of money is more like the latter than the former. 
...
where $\widehat Y(X)$ is the predicted value of $Y$ for a given value of $X$. This is result is derived in Section \@ref(derivation-9) below (optional).
...
The approximate interpretation of the regression coefficients in the log-linear model is that a one unit of change in $X$ is expected to yield a $(b \times 100)\%$ change in $Y$. This interpretation only applies with $b$ is small (less than 1 in absolute value)

The exact interpretation is that a one unit of change in $X$ is expected to yield a $((\exp(b) - 1)  \times 100)\%$ change in $Y$, and this applies for any value of $b$. 

### Derivation of Equation \@ref(eq:expb)*
...
This section works through log-linear regression using the `Wages.Rdata` example data set (source: Weinberg & Abramowitz (2017) Statistics using Stata: An integrative approach. Cambridge University Press). The dataset was collected from 200 individuals in 1985 and contains the following variables.   
