...
* The black dashed line shows how much closer we get to the student's Reading Achievement score ($Y$) by considering the predicted values ($\widehat Y$) instead of the overall mean ($\bar Y$). This represents the extent to which this student's Reading Achievement is explained by the linear relationship with SES. 

* The pink dashed line is the regression residual, which was introduced in Section \@ref(regression-line-2). This is the variation in Reading Achievement that is "left over" after considering the linear relationship with SES. 
...
The R-squared statistic averages the variation in Reading Achievement associated with SES (i.e., the black dashed line) relative to the total variation in Reading Achievement (i.e., black + pink) for all students in the sample. R-squared is the a widely used statistic in regression analysis, so we will be seeing it a lot. Some authors call it the "coefficient of determination" instead of R-squared. 
...
```{r, fig.cap = 'Reading Achievement and SES (NELS88).', fig.align = 'center'}
# Scatter plot
plot(x = ses, y = achmat08, col = "#4B9CD3", ylab = "Reading Achievement (Grade 8)", xlab = "SES")
...
This correlation means that eighth graders from more well-off families (higher SES) also tended to do better in reading (higher Reading Achievement). 
