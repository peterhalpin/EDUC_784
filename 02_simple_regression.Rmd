...
The problem in all of this is to give precise meaning to the terms "close" and "far". This is what we do when setting the significance level of the test, usually denoted $\alpha$. This number represents our tolerance for false negatives or Type I errors -- i.e., for incorrectly concluding that the null hypothesis is true (no effect). When we set $\alpha$ to a small number, we are saying that we want the probability of a false negative to be small. This means we are going to need strong evidence (i.e., a large value of $t$) before we reject the null hypothesis. Still, the question is "how large?"
