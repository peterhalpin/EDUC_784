...
The test of the slopes of the simple trends for the example are reported below. As stated, these aren't super interesting in the context of our example, but you should **check your understanding of simple trends by writing down an interpretation of the output below**. 
...
* Because the model in @eq-prek-ses-5 does not include an interaction, we already know that it implies that the effect of pre-K is constant over levels of SES.  Below we show that effect of Pre-K for SES = 2 is the same as the effect for SES = 1. The same approach can be used to show the effect is constant over all levels of SES. Note that this implication of the model is inconsistent with what is shown in @fig-prek-by-ses-5, and so we will improve on this model by adding an interaction in the following section.
...
Combined with the previous equation, this equation says that the difference between the red and blue lines in the second column of @fig-prek-by-ses-5 is the same as the difference in the first column -- i.e., they both equal $b_1$. This is what it means for there to be no interaction between two categorical predictors. 
...
We have just seen that @eq-prek-ses-5 implies that the effect of Pre-K is constant over levels SES, and vise versa. In order to address our research question about whether the relationship between Pre-K attendance and Math Achievement depends on children's SES, we will need to add something to the model -- an interaction (surprise!). 
