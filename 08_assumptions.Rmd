...
The following output shows the results for the heteroskedastic (cone-shaped) example data, using both the regular standard error and HC standard errors. 
...
As you can see, the regression coefficients are the same in both cases. However, the HC standard errors for the regression slope is quite a bit larger, with corresponding changes to the t-test and p-value. The HC standard error for the itnercept also changes, but not by very much. 
