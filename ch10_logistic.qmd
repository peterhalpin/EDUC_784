...
The previous section discussed the interpretation of simple logistic regression.
In this section we discuss how to get the numbers in the “Estimate” column of the R `summary()` table, and in the next section we discuss how to compute statistical tests and confidence intervals. 
...
$$ L = p \times CHD + (1 - p) \times (1 - CHD) $${#eq-Li}
...
Note that the confidence intervals produced by $R$ use the likelihood rather than the Wald test. So, the problems we discussed with the Wald test don't apply to these confidence intervals. (You can get Wald confidence intervals form the `confint` function, but this is not the default.)
...
$$ L = p_i \times CHD_i + (1 - p_i) \times (1 - CHD_i) $$
