...
<!-- add content on interpretation of subscripts in correlation matrix
add venn diagram relating b-weights and R-squared? 
-->
...
In order to represent correlation matrix among a single outcome ($Y$) and two predictors ($X_1$ and $X_2$) we will use the following notation. 
...
This notation is a bit tricky to understand, but the basic idea is that each correlation coefficient has two subscripts. The subscripts tell us which two variables are being correlated. For the outcome variable, we use the subscript $Y$, and for the two predictors we use the subscripts $1$ and $2$. As with the above example, the values below the diagonal mirror the values above the diagonal and the values on the diagonal are all equal to 1. So,  we really just need the three correlations show in the matrix below  
...
The three correlation are interpreted as follows: 
...
Just like simple regression, the residual for Equation \@ref(eq:yhat-4) is defined as $e = Y - \widehat Y$ and the model can be equivalently written as $Y = \widehat Y + e$. Also, remember that to write out the model using the variable names in places of $Y$ and $X$ if that helps makes things clear.   
...
Solving the minization problem (taking derivatives) leads to the following equations for the regression coefficients (remember, the subscript $1$ denotes the first predictor and the subscript $2$ denotes the second predictor)
...
As promised, these equations are more complicated than for simple regression :) The next section addresses the interpretation of the regression coefficients. But for now, the main thing to note is that multiple regression
...
So, this why we interpret the regression coefficients in multiple regression differently than simple regression. In multiple regression, we interpret the "effect" of one predictor while holding the other predictor(s) constant. 


### "Controlling for the other predictor"

Another interpretation of the regression coefficients is in terms of equations in for $b_1$ and $b_2$ presented in Section \@ref(ols-4).  For example, the equation for $b_1$ (the regression coefficient)
...
In general, the formulas for the regression coefficients in the two-predictor model are more complicated because they are "control for" or "account for" the relationship between the predictors. In simple regression, we only had one predictor, so we didn't need to account for how the predictors were related. 
...
However, the lm function in R does not provide an option to report standardized output. So, if you want to get the $\beta$ coefficients in R, its easiest to just standardized the variables first and then do regression with the standardized variables. 

The interpretation of the $\beta-$ coefficients is in terms of the standard deviation units of both the $Y$ variable and the $X$ variable -- e.g., increasing $X_j$ by one standard deviation increases $\hat Y$ by $\beta_j$ standard deviations (holding the other predictor(s) constant). 

For the ECLS example, the $\beta$-weights are reported below. Notice that, standardizing the coefficients doesn't change the t-tests, p-values, R-squared, its F-test. It just changes the scale of the regression coefficients.  
...
The circles represent the variance of each variable and the overlap between circles represents their shared variance. If we do two simple regression and the up the R-squared values, we would double count the area labelled "b". When we compute R-squared in  multiple regression, we only count that area once. 
<!--
\begin{align}
...
But for the squared correlation, $r^2$, the mean of the sampling distribution is slightly above zero because all of the random deviations are now positive (because they have been squared). Since the expected value of the $r^2$ is greater than the  population ($\rho = 0$),  we say that $r^2$ is an upwardly biased (i.e., too large) estimate of $\rho^2$. 
