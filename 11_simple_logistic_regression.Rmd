...
If we regress CHD on age using linear regression (i.e., the linear probability model), our diagnostic plots and output look like this: 
...
Then, once we are the logit, we can start doing logistic regression.
...
You might hear all of these terms (i.e., proportion, percentage, rate, mean, probability) used in connection with logistic regression. But, they are all just different ways of referring the rightmost column of Figure \@ref(fig:props). I will try to make a point of using all of these terms so you get used to interpreting them in this context :)  
...
So, let's see how to get from our S-shaped logistic function of $x$ to a linear function of $x$. A little algebra with \@ref(eq:logistic) shows that we can re-express the logistic function in terms of the odds: 
...
Just like the log-linear model, we report the results of our analysis in terms of proportional (or percentage) change in the odds of CHD associated with each additional year of age: 

* $(\exp(.11) - 1) \times 100 = 11.67$, so a persons' odds of CHD increase 11.67% for each additional year of age. 

In many logistic regression applications, the OR is reported instead of the regression coefficients from the logit model. <! -- We will discuss how to compute confidence intervals for the OR in Section \@ref(inference-11). -->
...
to find the age at which someone has equal odds of CHD. Doing the algebra shows that
...
Another interpretation is in terms of the slope of the straight line (tangent) through the point $p(CHD) = .5$. This slope of this line describes the approximate rate of change in the probability of CHD for people who are "close to" the age of equal odds.  
...
* In terms of predicted probabilities
    ...
    * Risk ratios can be computed to compare the predicted probabilities at different ages. 
...
```{r}
ci.b <- confint(mod2)
ci.table <- cbind(coefs = coef(mod1), ci.b)
exp(ci.table)
```
...
We will go through a worked example of multiple logistic regression in class together, focusing on the interpretation of the model coefficients and models with interactions. 
...
This section will be added before class, and we will work through the exercises together during class time. 
